...
class ReIDEngine:
    def __init__(self):
        ...
        self.debug_mode = True  # Enable debug logging by default
        self.similarity_threshold = 0.4  # Lowered from 0.6 - minimum similarity to consider same person
    ...
    async def initialize(self, model_path=None):
        ...
        try:
            ...
            self.log('ReID engine initialized with OSNet ONNX model (Single Threaded)')
    ...
    async def process_detection(self, image_buffer: bytes, camera_id: str,
        ...
        if match and match[1] >= self.similarity_threshold:
            ...
            self.log(f'✓ ReID MATCH: {person_id} on {camera_name} (similarity: {match[1]:.3f}, threshold: {self.similarity_threshold})')
            ...
            if match:
                self.log(f'✗ NO MATCH: Best similarity {match[1]:.3f} < threshold {self.similarity_threshold} - creating new person {person_id}')
            else:
                self.log(f'✗ NO MATCH: No tracked persons found - creating new person {person_id}')
            ...
            self.log(f'NEW PERSON: {person_id} detected on {camera_name} (total tracked: {len(self.tracked_persons)})')
